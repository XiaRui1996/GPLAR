...
from layers_q import SVGPLayer
...
class GPLAR(GPLARBase):
    ...
    def _init_layers(self, X, Y, Z, q_sqrt_initial, kernels, 
        ...
        for i in range(self.num_outputs):
            layer = Layer(kernels[i], inducing_inputs, Z[:,self.m+i], q_sqrt_initial[:,i], mean_function, white=white)
            layers.append(layer)
...
class GPLARmodel(GPLAR):
    def __init__(self, X, Y, M, gpar, likelihoods=None,
                 ...
                 nonlinear=True, nonlinear_scale=0.1,nonlinear_dependent=False,
                 rq=False,
                 markov=None,
    ...
    def _initialize_inducing_locations_old(self, X, Y, M): #M is number of inducing points per layer
    ...
    def _initialize_inducing_locations(self, X, Y, M):
        Z = np.linspace(np.min(X), np.max(X), M).reshape(-1,1)
        return Z
