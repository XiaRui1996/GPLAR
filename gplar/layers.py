...
class SVGPLayer(Layer):
    ...
    def __init__(self, kernel, inducing_variables, q_mu_initial, 
                 mean_function,white=False, **kwargs):
        super().__init__(**kwargs)

        self.inducing_points = inducingpoint_wrapper(inducing_variables)
        gpflow.set_trainable(self.inducing_points, False)
    ...
    def KL(self):
        ...
        return kullback_leiblers.prior_kl(self.inducing_points, self.kernel,
                self.q_mu, self.q_sqrt[None,:,:], whiten=self.white)
    
    def update_inducing_points(self, last_q_mu):
